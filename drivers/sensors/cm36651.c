...
#ifdef CONFIG_MACH_S3VE
		ps_reg_setting[1][1] = 0x0f;
#else
		ps_reg_setting[1][1] = 0x09;
#endif
	} else {
		ps_reg_setting[2][1] = cm36651->prox_cal = 0;
			     ...
			     const struct i2c_device_id *id)
{
	...
	err =  sensors_create_symlink(&cm36651->proximity_input_dev->dev.kobj, cm36651->proximity_input_dev->name);
	if (err) {
	...
	err =  sensors_create_symlink(&cm36651->light_input_dev->dev.kobj, cm36651->light_input_dev->name);
	if (err) {
...
err_devicetree:
	...
	err_sensors_create_symlink_proximity:
		sensors_remove_symlink(&cm36651->proximity_input_dev->dev.kobj, cm36651->proximity_input_dev->name);

	err_sensors_create_symlink_light:
		sensors_remove_symlink(&cm36651->light_input_dev->dev.kobj, cm36651->light_input_dev->name);
